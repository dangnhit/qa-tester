# Production-Readiness Plan — `@gwinnguyen/qa-skills`
...
**Phase 7 — Lane 2 (2 weeks). LANDED.**
- `qa-skill execute playwright -- <args>`: runtime spawns the runner, captures exit + JSON reporter, records `runtime-observed` + `commitSha` + `specTreeSha256`. ~~Dirty spec tree → REFUSED or `agent-draft`.~~ **Ruled REFUSED, with no downgrade and no `--allow-dirty`.** The artifact carrying the anchor is immutable and requires `commitSha`, so an `agent-draft` fallback would still record a commit identity describing a tree that is not the tree that ran — the downgrade does not weaken the claim, it just stops anyone checking it. Register one batched `test-result` + ~~the raw reporter JSON as evidence (checksummed)~~ **a *sanitized projection* of the reporter JSON as evidence (checksummed)** — **Sanitized Raw Evidence**, CONTEXT.md:275-277. The raw file carries `config.argv` and, for an ordinary web project, `config.webServer.env`, so registering it verbatim would put a resolved secret in an immutable artifact; it stays unregistered on disk at the `runnerWorkingDir` the command prints.
- **What the plan did not anticipate, and a reader now needs.** The producer also refuses a spec the report places outside the anchored directory (the ordinary cause is a `playwright.config` whose `testDir` is broader than `--spec-dir`; the adversarial one is a `--config` after `--` aimed at an unreviewed tree), and refuses an anchor that moved during the run — both halves of it, each independently reachable. What it cannot do is vouch for the report's *contents*: the observed process writes that file, and every module deciding what goes in it can sit outside the anchor. Lane 2 is an anchoring and provenance mechanism, not a sandbox.
- ~~**Two prerequisites, both blocking, both inherited from Phase 6.**~~ **Both landed in `test-result-batch` 3.0.0, before any producer exists — which was the point.** A producer landing first would have turned each from a latent wart into a live mis-credit site. They were stated here rather than only in TSDoc on `release-gate.ts` / `evaluate-workspace-coverage.ts` because this bullet is the contract the next phase executes against, and those files need never be opened to write a producer. What the producer must now emit:
  1. **The attempt's Execution Surface is read, not hardcoded.** Each entry declares its own `executionSurface` (required; the `coverage-obligation` enum), and both readers read it with no fallback — the fail-OPEN one drops an entry that names none, the fail-CLOSED one rejects it. A producer must stamp the surface the suite actually ran on; a `unit` entry can no longer credit a browser obligation. *As shipped, the surface's source is an in-spec identity tag — `[qa:<testCaseId>/<revisionId>/<instanceId>@<surface>]` in the `test(...)` title — chosen so the declaration sits **inside** the anchored tree and `specTreeSha256` covers it. A `--surface` flag would have sat outside the anchor, where it could be changed without dirtying the tree.*
  2. **The viewport is observed, not inherited from the plan.** Each browser entry declares its own `viewport` (required on `browser`, forbidden elsewhere, as is `observedEngine`), and both readers compare that value rather than `test-case.coverage.viewport`. A producer must report the geometry the runner actually rendered at; an entry can no longer credit a geometry nothing rendered at. Lane 1 still derives its viewport from the declaration, correctly — `createBrowserAttemptSession` SETS the live context from it and the DSL has no resize action. *As shipped, **lane 2 emits no browser entry at all**, so this obligation binds its producer vacuously — which is a change in the obligation, not a reason the prerequisite did not matter. Two independent causes: Playwright 1.61.1's JSON reporter serializes each project from a ten-key allowlist that never includes `use` (`JSONReporter._serializeReport`), so neither the engine nor the geometry is in the report to read; and CONTEXT.md:444 keeps the browser surface in the runtime's own hands, lane 2 reaching every **other** surface. So a `@browser` tag is refused rather than credited with a guessed geometry. The prerequisite is still what makes that refusal structural rather than merely producer policy: `viewport` and `observedEngine` are forbidden outside `browser`, so a lane-2 entry cannot carry either even if a later producer tried.*
- ~~Also lift the **human checkpoint** for `qa-skill attestation record` here~~ **Landed as `AWAITING_HUMAN_INPUT`.** Phase 6 shipped the artifact, the producer and the credit path, but no run position where a person could record an attestation between the obligation being registered and the gate being snapshotted — so a `required` manual Accessibility Obligation was unsatisfiable. `qa-skill workflow run` now stops immediately before the operation the missing record gates and returns `AWAITING_HUMAN_INPUT` without writing a gate or finalizing, resumable by `resumeRunId`. **It serves both human records, not only the attestation**: `execute-browser-test` for an approval, `generate-qa-report` for an attestation. A single run can therefore stop twice, and recording the first record and resuming can legitimately return the second.
