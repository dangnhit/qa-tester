...
- **Two prerequisites, both blocking, both inherited from Phase 6.** A `test-result-batch` producer landing before these are fixed turns each of them from a latent wart into a live mis-credit site, because today no producer writes the shape that reaches them. They are stated here rather than only in TSDoc on `release-gate.ts` / `evaluate-workspace-coverage.ts` because this bullet is the contract the next phase executes against, and those files need never be opened to write a producer.
  1. **The attempt's Execution Surface must be read, not hardcoded.** Both readers flatten a batch entry with a literal `executionSurface: "browser"` (`asAttempt` in `src/reporting/release-gate.ts`, `dimensions()` in `src/operations/evaluate-workspace-coverage.ts`). It is correct only while `test-case.coverage` is browser-only by schema. Make it a real read off the observed-execution record first, or a non-browser batch entry will credit a browser obligation.
  2. **The viewport must be observed, not inherited from the plan.** Both readers take an attempt's viewport from `test-case.coverage` — the geometry the test case *declared* — which is the same declared-label-credits-itself defect Phase 6 removed for the engine (`observedEngine`, CONTEXT.md:442). Lane 2 reports a real viewport; read it from the execution record, so an entry cannot credit a geometry nothing rendered at.
