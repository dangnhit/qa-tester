...
- When a regression selection exists, its ordered selected decisions and canonical testcase relationships are the sole authority for checkpoint execution cases; excluded cases, additions, omissions, and reordered state references fail reopen even after checkpoint and manifest checksums are recomputed. Hydration derives those IDs from the verified selection output instead of accepting the serialized state list.
