...
**READY_FOR_REVIEW.** The production runner performs one `operationsForMode(mode)` iteration through a complete closed typed adapter map. Each adapter owns its labelled work and typed postcondition; immutable checkpoints are appended only after that postcondition succeeds. The public Chromium tracer proves the exact order, exactly one adapter/postcondition invocation per operation, and resume at the failed operation without rerunning earlier checkpoints. Regression decisions bind exact testcase revision *and instance*, and retest source reproduction retains repeated scenario occurrences rather than deduplicating them.

Public Chromium acceptance covers both multi-scenario verdicts: repeated attempts of one canonical source scenario persist and reopen as `INTERMITTENT`; two distinct source testcase instances/scenario IDs replay before regression, produce one pass and one failure, and persist/reopen as `PARTIALLY_FIXED`. Every persisted reproduction occurrence carries its source testcase and source attempt artifact IDs. Rechecksummed tampering of either source artifact ID or canonical scenario mapping rejects on reopen, alongside existing instance/attempt/verdict checks. Validation: generated types, typecheck, focused runtime E2E, full suite, lint, build, and `git diff --check`.

## Review correction: durable output and resume consistency

- `WorkflowResult.outputs` is now a public key-correlated mapped object, with operation input/output maps exported from the package entrypoint. Each value is reconstructed from canonical artifact references: execution and reproduction return test-result records; evidence collection returns evidence/gap records; planning ingestion returns imported records; data preparation returns its manifest; report generation returns gate and report records.
- Checkpoints persist a checksummed internal state snapshot for imported artifacts, selected execution cases, source/reproduction/regression attempt references, selection, charter, and exploratory findings. Resume reopens and revalidates every reference before continuing, then restores the exact selected work rather than rediscovering broader workspace state.
- The adapter loop invokes each postcondition once. Checkpoint advancement receives only an already-verified output and state snapshot. A failed execution postcondition resumes using the already-registered result attempt and adds no duplicate attempt.
- Retest source scenarios come only from the selected bug revision's recorded reproduction provenance. Reopen validation performs pairwise source-attempt/testcase identity checks and recomputes each scenario identity, rejecting rechecksummed swapped pairs.
- Regression selection records all changes covered by a testcase, retaining the highest-priority mapping source plus deterministic combined rationale. Full-mode bug generation processes every failed attempt, requiring an eligible product bug or typed incident before checkpointing.

## Review correction: evidence, checkpoint, and failure-disposition boundaries

- Runtime evidence descriptors and attempt-scoped Evidence Gaps now carry the exact testcase revision and instance and have exactly one `test-result` relationship. Workspace registration and reopen reject swapped result relationships, rechecksummed identity changes, or attempt A/B reassignment. Non-attempt operational gaps explicitly use `scope: operational` and never carry an attempt ID.
- Checkpoints use the closed operation-name enum and are immutable prefixes of `operationsForMode(mode)`. Their output keys exactly equal the completed prefix, output references are type-checked against the declared operation output contract, state references are canonical-checksummed and type-derived, and successor relationships contain precisely the predecessor plus newly checkpointed outputs. Resume rehydrates and re-runs every checkpointed operation’s read-only postcondition before an adapter can be skipped.
- When a regression selection exists, its ordered selected decisions and canonical testcase relationships are the sole authority for checkpoint execution cases; excluded cases, additions, omissions, and reordered state references fail reopen even after checkpoint and manifest checksums are recomputed. This field uses ordered comparison deliberately; only intrinsically unordered fact sets use set-style comparison. Hydration derives those IDs from the verified selection output instead of accepting the serialized state list.
- Regression and retest now run `generate-bug-report` after evidence and before the report or retest verdict. The postcondition rejects a workflow with any failed test attempt lacking an eligible product-bug revision or typed incident disposition.
