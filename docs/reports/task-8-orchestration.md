...
**BLOCKED.** Corrective hardening now registers the immutable test result before any live evidence hook, emits final (non-pending) runtime evidence, prevents protected console/network/log payloads from persisting without a deterministic registered scrubber, and records a governed Evidence Gap instead. Workflow checkpoints now use an immutable revision chain with completed operations and checksummed output references; retest change-scope and source-bug dependencies are preflighted before reproduction; scenario IDs derive from canonical testcase identity rather than caller labels.

Focused validation is green for evidence, workspace durability, retest verdicts, and the public Chromium runtime matrix (75 tests), followed by generated types, typecheck, lint, build, and `git diff --check`. This is not ready for review yet: the public runner still needs to be fully converted to the closed typed adapter-map iteration, source reproduction sets still need multi-scenario execution at the public runtime boundary, and charter exploration still needs its complete action/target/stop/safety runtime contract. The complete suite must then pass twice, including its package-consumer build tracer.
