...
describe("workflow mode operation plans", () => {
  ...
  it("returns the finalized metadata outcome when a passing attempt still fails the artifact profile", async () => {
    ...
    try {
      ...
      const testcase = await workspace.registerArtifactValue({ type: "test-case", relationships: [plan.id], value: { artifactType: "test-case", schemaVersion: "2.0.0", producerVersion: "1.0.0", testCaseId: "TC-INVALID-PROFILE", revisionId: "REV-INVALID-PROFILE", instanceId: "INSTANCE-INVALID-PROFILE", title: "Open", steps: [{ id: "open", action: "navigate", sideEffect: "none" }], coverage: { requirementId: "REQ-INVALID-PROFILE", role: "member", behavior: "open", browser: "chromium", viewport: { width: 1280, height: 720 }, accessibilityMethod: null, risk: "low", outcome: "Open" } } });
      await workspace.registerArtifactValue({ type: "test-result", relationships: [testcase.id], value: { artifactType: "test-result", schemaVersion: "2.0.0", producerVersion: "1.0.0", attemptId: "ATT-INVALID-PROFILE", runId: workspace.runId, testCaseId: "TC-INVALID-PROFILE", testCaseRevisionId: "REV-INVALID-PROFILE", testCaseInstanceId: "INSTANCE-INVALID-PROFILE", status: "PASSED", failureClassification: "NONE", observedEngine: "chromium", steps: [{ stepId: "open", status: "PASSED", durationMs: 1 }], startedAt: "2026-07-23T00:00:00.000Z", finishedAt: "2026-07-23T00:00:01.000Z" } });
