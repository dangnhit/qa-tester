...
async function sourceBundle(root: string, options: { sourceBug?: boolean | "intermittent" | "partial"; includeExcluded?: boolean } = {}): Promise<CanonicalPlanBundleRef & { sourceBug?: { artifactId: string; sha256: string; bugId: string } }> {
  ...
  const testcase = await source.registerArtifactValue({ type: "test-case", relationships: [plan.id], value: {
    artifactType: "test-case", schemaVersion: "2.0.0", producerVersion: "1.0.0", testCaseId: "TC-RUNTIME", revisionId: "REV-RUNTIME", instanceId: "INSTANCE-RUNTIME", title: "Save email", steps: [{ id: "plan-open", action: "navigate", sideEffect: "none" }], coverage: { requirementId: "REQ-RUNTIME", role: "member", behavior: "save email", browser: "chromium", viewport: { width: 1280, height: 720 }, accessibilityMethod: null, risk: "low", outcome: "Saved" },
  } });
  const excluded = options.includeExcluded ? await source.registerArtifactValue({ type: "test-case", relationships: [plan.id], value: {
    artifactType: "test-case", schemaVersion: "2.0.0", producerVersion: "1.0.0", testCaseId: "TC-EXCLUDED", revisionId: "REV-EXCLUDED", instanceId: "INSTANCE-EXCLUDED", title: "Excluded browser case", steps: [{ id: "plan-open-excluded", action: "navigate", sideEffect: "none" }], coverage: { requirementId: "REQ-RUNTIME", role: "member", behavior: "excluded", browser: "chromium", viewport: { width: 1280, height: 720 }, accessibilityMethod: null, risk: "low", outcome: "Saved" }, regressionIndex: { requirementIds: [], codeSurfaces: ["excluded-surface"], declaredDependencies: [], gitPaths: [], userScope: [] },
  } }) : undefined;
  const second = options.sourceBug === "partial" ? await source.registerArtifactValue({ type: "test-case", relationships: [plan.id], value: { artifactType: "test-case", schemaVersion: "2.0.0", producerVersion: "1.0.0", testCaseId: "TC-RUNTIME", revisionId: "REV-RUNTIME", instanceId: "INSTANCE-RUNTIME-SECOND", title: "Save email second instance", steps: [{ id: "plan-open-2", action: "navigate", sideEffect: "none" }], coverage: { requirementId: "REQ-RUNTIME", role: "member", behavior: "save email second", browser: "chromium", viewport: { width: 1280, height: 720 }, accessibilityMethod: null, risk: "low", outcome: "Broken" } } }) : undefined;
  const coverage = await source.registerArtifactValue({ type: "coverage-obligation", relationships: [requirement.id], value: {
    artifactType: "coverage-obligation", schemaVersion: "3.0.0", producerVersion: "1.0.0", obligationId: "COV-RUNTIME", requirementAnalysisArtifactId: requirement.id, requirementId: "REQ-RUNTIME", role: "member", behavior: "save email", executionSurface: "browser", browser: "chromium", viewport: { width: 1280, height: 720 }, accessibilityMethod: null, risk: "low", outcome: "Saved", required: true,
  } });
  ...
  const sourceAttempt = await source.registerArtifactValue({ type: "test-result", relationships: [testcase.id], value: { artifactType: "test-result", schemaVersion: "2.0.0", producerVersion: "1.0.0", attemptId: "ATT-SOURCE-BUG", runId: source.runId, testCaseId: "TC-RUNTIME", testCaseRevisionId: "REV-RUNTIME", testCaseInstanceId: "INSTANCE-RUNTIME", status: "FAILED", failureClassification: "PRODUCT_DEFECT", observedEngine: "chromium", steps: sourceSteps, startedAt: "2026-07-23T00:00:00.000Z", finishedAt: "2026-07-23T00:01:00.000Z" } });
  if (options.sourceBug === "intermittent") await source.registerArtifactValue({ type: "test-result", relationships: [testcase.id], value: { artifactType: "test-result", schemaVersion: "2.0.0", producerVersion: "1.0.0", attemptId: "ATT-SOURCE-BUG-REPEAT", runId: source.runId, testCaseId: "TC-RUNTIME", testCaseRevisionId: "REV-RUNTIME", testCaseInstanceId: "INSTANCE-RUNTIME", status: "FAILED", failureClassification: "PRODUCT_DEFECT", observedEngine: "chromium", steps: sourceSteps, startedAt: "2026-07-23T00:02:00.000Z", finishedAt: "2026-07-23T00:03:00.000Z" } });
    if (options.sourceBug === "partial" && second) await source.registerArtifactValue({ type: "test-result", relationships: [second.id], value: { artifactType: "test-result", schemaVersion: "2.0.0", producerVersion: "1.0.0", attemptId: "ATT-SOURCE-BUG-SECOND", runId: source.runId, testCaseId: "TC-RUNTIME", testCaseRevisionId: "REV-RUNTIME", testCaseInstanceId: "INSTANCE-RUNTIME-SECOND", status: "FAILED", failureClassification: "PRODUCT_DEFECT", observedEngine: "chromium", steps: sourceSteps, startedAt: "2026-07-23T00:02:00.000Z", finishedAt: "2026-07-23T00:03:00.000Z" } });
    await source.registerEvidenceBundle({ binaries: [{ filename: "source-bug.txt", contents: Buffer.from("source browser failure"), mediaType: "text/plain", captureType: "log" }], relationships: [sourceAttempt.id], descriptor: (binaries) => ({ artifactType: "evidence", schemaVersion: "3.0.0", producerVersion: "1.0.0", evidenceId: "01K0ABCDEFGHJKMNPQRSTVWXYZ", runId: source.runId, subject: { kind: "attempt", attemptId: "ATT-SOURCE-BUG", testCaseId: "TC-RUNTIME", testCaseRevisionId: "REV-RUNTIME", testCaseInstanceId: "INSTANCE-RUNTIME" }, kind: "log", capturedAt: "2026-07-23T00:01:00.000Z", sha256: binaries[0]!.sha256, relativePath: binaries[0]!.relativePath, mediaType: "text/plain", binaryArtifactIds: binaries.map((binary) => binary.id), binaryArtifacts: binaries.map((binary) => ({ id: binary.id, relativePath: binary.relativePath, sha256: binary.sha256, mediaType: binary.mediaType })), telemetryFindings: [{ kind: "console", level: "error", message: "source browser failure" }], provenance: { captureType: "log", url: baseUrl, browser: "chromium", build: "fixture", capturedAt: "2026-07-23T00:01:00.000Z", testcaseId: "TC-RUNTIME" } }) });
...
describe("public runtime QA Tester", () => {
  ...
  it("does not register a spurious evidence gap when resuming execute-browser-test for an already-registered attempt that already has evidence", async () => {
    ...
    const registeredResult = await workspace.registerArtifactValue({
      ...
      value: {
        artifactType: "test-result", schemaVersion: "2.0.0", producerVersion: "1.0.0", attemptId, runId: workspace.runId,
    ...
    await workspace.registerEvidenceBundle({
      ...
      descriptor: (binaries) => ({
        artifactType: "evidence", schemaVersion: "3.0.0", producerVersion: "1.0.0", evidenceId: createEntityId(), runId: workspace.runId,
