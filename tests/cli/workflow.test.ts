...
async function planningOnlyRun(root: string, envPath: string, identity: typeof bugIdentity): Promise<string> {
  ...
  const testcase = { artifactType: "test-case", schemaVersion: "3.0.0", producerVersion: "1.0.0", testCaseId: identity.testCaseId, revisionId: identity.revisionId, instanceId: identity.instanceId, title: "Save", steps: [{ id: "open", action: "navigate", sideEffect: "none" }], coverage: { requirementId: identity.requirementId, role: "member", behavior: "save", browser: "chromium", viewport: { width: 1280, height: 720 }, accessibilityMethod: null, risk: "low", outcome: "Saved" } };
  const coverage = { artifactType: "coverage-obligation", schemaVersion: "4.0.0", producerVersion: "1.0.0", obligationId: `COV-${identity.testCaseId}`, requirementAnalysisArtifactId: "replaced-atomically", requirementId: identity.requirementId, role: "member", behavior: "save", executionSurface: "browser", browser: "chromium", viewport: { width: 1280, height: 720 }, accessibilityMethod: null, risk: "low", outcome: "Saved", required: true };
  await Promise.all([
...
async function executedRunWithBug(root: string, options: { bugs?: number } = {}): Promise<{ runId: string; bugArtifactId: string; bugSha256: string }> {
  ...
  for (const identity of identities) {
    ...
    const testcase = await source.registerArtifactValue({ type: "test-case", relationships: [plan.id], value: {
      artifactType: "test-case", schemaVersion: "3.0.0", producerVersion: "1.0.0", testCaseId: identity.testCaseId, revisionId: identity.revisionId, instanceId: identity.instanceId, title: "Save", steps: [{ id: "open", action: "navigate", sideEffect: "none" }],
    ...
    const attempt = await source.registerArtifactValue({ type: "test-result", relationships: [testcase.id], value: {
      artifactType: "test-result", schemaVersion: "3.0.0", producerVersion: "1.0.0", attemptId, runId: source.runId, testCaseId: identity.testCaseId, testCaseRevisionId: identity.revisionId, testCaseInstanceId: identity.instanceId,
...
describe("workflow scaffold", () => {
  it("bootstraps the first complete terminal planning bundle from explicit canonical files", async () => {
    ...
    const testcase = { artifactType: "test-case", schemaVersion: "3.0.0", producerVersion: "1.0.0", testCaseId: "TC", revisionId: "REV", instanceId: "INSTANCE", title: "Save", steps: [{ id: "open", action: "navigate", sideEffect: "none" }], coverage: { requirementId: "REQ", role: "member", behavior: "save", browser: "chromium", viewport: { width: 1280, height: 720 }, accessibilityMethod: null, risk: "low", outcome: "Saved" } };
    const coverage = { artifactType: "coverage-obligation", schemaVersion: "4.0.0", producerVersion: "1.0.0", obligationId: "COV", requirementAnalysisArtifactId: "replaced-atomically", requirementId: "REQ", role: "member", behavior: "save", executionSurface: "browser", browser: "chromium", viewport: { width: 1280, height: 720 }, accessibilityMethod: null, risk: "low", outcome: "Saved", required: true };
    await Promise.all([
  ...
  it("creates parseable plan and source-bound inputs without scanning for a latest run", async () => {
    ...
    await workspace.registerArtifactValue({ type: "test-case", relationships: [planArtifact.id], value: { artifactType: "test-case", schemaVersion: "3.0.0", producerVersion: "1.0.0", testCaseId: "TC", revisionId: "REV", instanceId: "INSTANCE", title: "Save", steps: [{ id: "open", action: "navigate", sideEffect: "none" }], coverage: { requirementId: "REQ", role: "member", behavior: "save", browser: "chromium", viewport: { width: 1280, height: 720 }, accessibilityMethod: null, risk: "low", outcome: "Saved" } } });
    await workspace.registerArtifactValue({ type: "coverage-obligation", relationships: [requirement.id], value: { artifactType: "coverage-obligation", schemaVersion: "4.0.0", producerVersion: "1.0.0", obligationId: "COV", requirementAnalysisArtifactId: requirement.id, requirementId: "REQ", role: "member", behavior: "save", executionSurface: "browser", browser: "chromium", viewport: { width: 1280, height: 720 }, accessibilityMethod: null, risk: "low", outcome: "Saved", required: true } });
