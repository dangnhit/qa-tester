 ...
 *
 * `runner-report` (added by evidence schema 3.0.0; current is 4.0.0) is lane 2's: one raw JSON report
 * for a whole Runtime-Observed Execution. It is deliberately the SAME token as the evidence `kind` and
 * the provenance `captureType`, because `matchesEvidencePrimary` requires all three to be equal.
 */
...
/** What an evidence record is *about* (the `subject` union, added by evidence schema 2.0.0; current
 *  evidence schema is 4.0.0). `attempt` is lane 1 — a runtime-driven browser attempt bound to one
 *  canonical `test-result`. `observed-execution` is lane 2 — an externally observed Playwright
 *  execution, whose `executionId` is the identity field of the `test-result-batch` artifact; it binds
 *  no attempt and therefore no `test-result`. */
