...
**A `FAILED` entry makes the run report failure, and produces no bug report.** The first half is the honesty rule: a run holding an observed failure finalizes `COMPLETED_WITH_FAILURES` and exits `1`, exactly as it would for a `test-result` that failed (`hasExecutionFailures`, `src/operations/run-workflow.ts`), and the failed case earns no coverage credit either, because `evaluateCoverage` (`src/planning/coverage.ts`) credits only `PASSED`. The second half is the same structural limit `retest` runs into: a bug report and a typed incident are both bound to an `attemptId`, and a batch entry carries only an `entryId`, so `generate-bug-report` has nothing to bind a defect to and does not invent one. What you get for a lane-2 failure is the run's terminal status plus the obligation it left unmet in the release gate — not a triaged defect. Drive the case in lane 1 if you need one.
